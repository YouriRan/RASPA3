 ...
 */
System::System(std::size_t id, ForceField forcefield, std::optional<SimulationBox> box, bool hasExternalField, double T,
...
{
  std::size_t idx = numberOfFrameworkAtoms;
  for (std::size_t cid = 0; cid < components.size(); ++cid)
  {
    for (std::size_t m = 0; m < numberOfMoleculesPerComponent[cid]; ++m)
    {
      for (std::size_t a = 0; a < components[cid].atoms.size(); ++a, ++idx)
      {
        atomData[idx].moleculeId = static_cast<std::uint16_t>(m);
        atomData[idx].componentId = static_cast<std::uint8_t>(cid);
      }
    }
  }
}
...
void System::sampleProperties(std::size_t currentBlock, std::size_t currentCycle)
{
  ...
  if (samplePDBMovie.has_value())
  {
    samplePDBMovie->update(forceField, systemId, simulationBox, spanOfMoleculeAtoms(), components,
                           numberOfMoleculesPerComponent, currentCycle);
  }
